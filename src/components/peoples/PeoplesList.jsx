...
import { getAsync, get_fil, get_all } from '../../features/peoples/peoplesSlice';
...
const PeoplesList = () => {
  ...
  const [currentUrl, setcurrentUrl] = useState(url.urlpeoples)  
    ...
    <SafeAreaView>
        ...
        <FlatList contentContainerStyle={{ paddingBottom: 250 }}            
            ...
            renderItem={({ item:item }) => (
              <Link to={'/peoplesdetail'} state={{item}}>
                <Peoples {...item} />
              </Link>
            )}            
